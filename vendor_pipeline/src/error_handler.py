...
def handle_error(error: Exception, exit_code: int = 1, *, exit_on_error: bool = False) -> None:
    """
    统一的错误处理函数，打印错误信息。
    ...
    Args:
        error: 异常对象
        exit_code: 退出码（仅在 exit_on_error=True 时使用）
        exit_on_error: 是否在错误处理后退出程序（默认为 False，安全模式）
    
    Raises:
        ChipSynthesisError: 如果 exit_on_error=False，将抛出异常而不是退出
    """
    # 构建简洁的错误消息
    error_lines = []
